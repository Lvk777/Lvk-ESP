...
local ConfigLibrary = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/Lvk777/Config-Library/main/Main.lua"))()
...
local Environment = getgenv().LvkDeveloperESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("Lvk_ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "Lvk_ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("Lvk_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "Lvk_ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "Lvk_ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "Lvk_ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().LvkDeveloperESP.DeveloperSettings = DeveloperSettings
	getgenv().LvkDeveloperESP.Settings = Settings
	getgenv().LvkDeveloperESP.Properties = Properties

	Environment = getgenv().LvkDeveloperESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Lvk_ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Lvk_ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
